...
public class ClassInfoController extends BaseController {
    ...
    public List<Map> findIdAndNameByRealName(@RequestParam(value = "q",defaultValue = "") String q){
        List<SisoClassInfoDto> classInfolist = sisoClassInfoService.selectIdsByName(q);
        List<Map> classIds = new ArrayList<>();
        for (SisoClassInfoDto c : classInfolist){
            Map classMap= new HashMap();
            classMap.put("id",c.getId());
            classMap.put("text",c.getClassName());
            classIds.add(classMap);
        }
        return classIds;
    }
