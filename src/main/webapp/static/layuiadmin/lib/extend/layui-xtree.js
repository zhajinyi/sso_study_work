...
layuiXtree.prototype.dataBind = function (d) {
    ...
    if (d.length > 0) {
        for (var i=0;i< d.length;i++) {
            ...
            _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + xtree_ischecked + xtree_isdisabled + ' value="' + d[i].value + '" lay-skin="primary" lay-filter="xtreeck' + _this._containerid + '"><span class="menuname" id="menu' + d[i].value + '">' + d[i].title + '</span>';
...
//全部选中的checkbox对象取消选中
layuiXtree.prototype.ClearAllChecked = function () {
    var _this = this;
    var cks = _this.getByClassName('layui-xtree-checkbox');
    for (var i = 0; i < cks.length; i++) {
        if (cks[i].checked) {
...
//根据value选中
layuiXtree.prototype.CheckedByValue = function (value) {
    var _this = this;
    var cks = _this.GetAllCheckBox();
    var arr = new Array();
    for (var i = 0; i < cks.length; i++) {
        if (cks[i].value == value) {
            cks[i].checked = true;
            cks[i].parentNode.getElementsByClassName('layui-form-checkbox')[0].classList.add('layui-form-checked');
            arr[0] = cks[i];
        }
    }
    return arr[0];
