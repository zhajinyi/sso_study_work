    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        if (!roleList.isEmpty()) {
            for(SysRoleDto role : roleList) {
                ...
                Set<SysRoleMenuKey> permissionSet = role.getPermissions();
                if (!permissionSet.isEmpty()) {
                    for (SysRoleMenuKey menuDto : permissionSet) {
                        String menuePerm = menuDto.getMenuPermission();
                        String menueID = menuDto.getMenuId();
                        String p = "";
                        if (menuePerm.indexOf(",")>0){
                            String[] mL = menuePerm.split(",");
