...
import static org.bureaumanager.bureaumanagerpro.sysConfig.config.Global.getConfig;

/**
 * @Author BaoLing
 * @Date: 2018/12/17
 * @Description PageBean工具类，用于接收分页结果
 * @Version 2.0
 */
public class PageBean<T> {

    protected int pageNo = 1; // 当前页码
    protected int pageSize = Integer.valueOf(getConfig("page.pageSize")); // 页面大小，设置为“-1”表示不进行分页（分页无效）
     ...
     */
    public PageBean(HttpServletRequest request, HttpServletResponse response){
     ...
     */
    public PageBean(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
     ...
     * @param count 数据条数
     */
    public PageBean(int pageNo, int pageSize, long count) {
     ...
     * @param list 本页数据对象列表
     */
    public PageBean(int pageNo, int pageSize, long count, List<T> list) {
        this.setCount(count);
